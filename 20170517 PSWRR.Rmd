...
gradcohort %>%
  ...
  summarise_all(funs(mean)) %>% summarise_at(vars(3:8), funs(percent)) %>% bind_cols(epso_cov, act_dist) %>% mutate(`District Name`=districtname) %>% 
    select(`District Name`,`Graduation Rate`,  `Avg. ACT (Highest)`, `% EPSO`, `% CTE`,`% BHN`,`% ED`, `% SWD`, `% EL`)  %>%  knitr::kable(longtable = TRUE, col.names = c("District Name","Grad. Rate", "Avg. ACT", "% EPSO", "% CTE", "% BHN","% ED", "% SWD", "% EL"),align="lrrrrrrrr" )
```

```{r `District by Type P1`, fig.height=3,  fig.align = 'center'}
StateRate3b <- dist %>%
    filter(page == "3b" & institution_type!= "Did not enroll") %>%
    select(pct_enroll_state, institution_type, GradYear) %>% 
    mutate(districtname = "Tennessee") %>% mutate(ps_pct_enroll= round(pct_enroll_state,0) ) %>% select(GradYear, districtname, institution_type, ps_pct_enroll)
   

dist %>%
    filter(page == "3b" & institution_type!= "Did not enroll") %>%
    mutate(ps_pct_enroll = round(100 * enroll_bin/hsgrad_denom, 0)) %>%
    select(GradYear, districtname, institution_type, ps_pct_enroll) %>% 
    bind_rows(StateRate3b) %>% arrange(districtname, desc(GradYear), desc(institution_type)) %>% 
  group_by(districtname,GradYear) %>% 
  mutate(`Total Enrollment` = sum(ps_pct_enroll ), `Cumulative Enrollment` = cumsum(ps_pct_enroll)) %>% 
  ungroup() %>% 
  mutate(pctElab = as.factor(ifelse(ps_pct_enroll>= 10, paste0(`ps_pct_enroll`, "%", sep = ""), ""))) %>%
    ggplot(aes(x =districtname, y = ps_pct_enroll, fill = institution_type)) +
        geom_col(position = "stack", width = .5, colour="black") +
        facet_wrap(~ as.factor(GradYear )) + 
        geom_text(aes( x= districtname, y = `Cumulative Enrollment`,label = pctElab, vjust=1), size = 3) +
       theme_hc() +
        ...
        scale_fill_manual( name = "", values = c("#e02f11", "#24336b", "#959595")) +
      geom_text(aes(x = districtname, y = 90,
             label = paste("Overall: \n", `Total Enrollment`, "%", sep = "")), size = 3, fontface="italic") +
        ylab("Percent of HS graduates enrolled") +
        xlab("") +
                ylim(0, 100)
...
\end{center}
->![](K:/Research and Policy/projects/reports/ACT Sankey Hardeman.png)<-
<!-- \begin{center} -->
<!-- \begin{figure}[Sankey] -->
<!--     \centering -->
<!--       \includegraphics[width=90mm]{"K:/Research and Policy/projects/reports/ACT Sankey Hardeman.png"} -->
<!--     \caption{ACT into Postsecondary \label{overflow}} -->
<!--     \end{figure} -->
<!-- \end{center} -->
\begin{center}
...
**Graduation Year**: This report uses the term Graduation Year to align with the On-Time Graduates with a regular diploma, based on when the group of students entered high school. 

**Postsecondary Enrollment**: A student is identified as having enrolled in a postsecondary institution if they enroll within 12 months of graduation. Ninety-eight percent of schools nationwide report enrollments.
...
**Postsecondary Completion**: Postsecondary completion documentation is shown for students in the 2007 and 2008 ninth grade graduating cohorts. Any degree-granting public institution that submits completion information to THEC or to NSC [add link] is included in this set of data points. As of `r format(Sys.time(), '%B %d, %Y')`, the most recent term with completion information available is Summer 2016. A student's most advanced degree is shown. 

### District comparisons  
This report provides comparisons to Tennessee school districts whose populations share similar demographic characteristics. Be respectful in your use of other school districts' information; do not share this information publicly.
