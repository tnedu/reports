...
gradcohort %>%
  ...
  select(`District Name`,`Graduation Rate`, `% CTE`, `Avg. ACT (Highest)`, `% EPSO`, `% BHN`,`% ED`, `% SWD`, `% EL`) %>%  knitr::kable(col.names = c("District Name","Grad. Rate", "% CTE", "Avg. ACT", "% EPSO", "% BHN","% ED", "% SWD", "% EL"),align="lrrrrrrrr", caption="2015 Graduates")
```
...
dist %>%
    ...
    ggplot(aes(x = institution_type, y = ps_pct_enroll, fill = institution_type)) +
        geom_bar(stat = "identity", position = position_dodge(-1), width = .75, colour="black", show.legend = FALSE) +
