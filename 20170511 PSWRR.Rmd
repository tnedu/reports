...
StateRate3b <- dist %>%
    ...
    select(pct_enroll_state, institution_type, cohortyear) %>% 
    mutate(districtname = "Tennessee") %>% mutate(ps_pct_enroll= round(pct_enroll_state,0) ) %>% select(cohortyear, districtname, institution_type, ps_pct_enroll)
   

dist %>%
    filter(page == "3b" & institution_type!= "Did not enroll") %>%
    mutate(ps_pct_enroll = round(100 * enroll_bin/hsgrad_denom, 0)) %>%
    select(cohortyear, districtname, institution_type, ps_pct_enroll) %>% 
    bind_rows(StateRate3b) %>% arrange(districtname, desc(cohortyear), desc(institution_type)) %>% 
  group_by(districtname,cohortyear) %>% 
  ...
  mutate(pctElab = as.factor(ifelse(ps_pct_enroll>= 10, paste0(`ps_pct_enroll`, "%", sep = ""), ""))) %>%
    ggplot(aes(x =districtname, y = ps_pct_enroll, fill = institution_type)) +
        geom_col(position = "stack", width = .75, colour="black") +
        facet_wrap(~ as.factor(cohortyear)) + 
        geom_text(aes( x= districtname, y = `Cumulative Enrollment`,label = pctElab, vjust=1), size = 2.5) +
       theme_hc() +
