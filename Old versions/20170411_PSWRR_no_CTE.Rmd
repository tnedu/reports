...
This page will contain a series of figures that in a clean way display for the most recent cohort. It will act as an introduction for the lay viewer. It will provide a brief overview of the graduating class as well as the key pieces of data..
...
dorder<-c("District Name" "Graduation\n Rate", "% BHN","% ED", "% SWD", "% EL", "Avg. ACT \n (Highest)", "% EPSO") %>%
    as_data_frame()
...
gradcohort %>%
  ...
  summarise_all(funs(mean)) %>% summarise_at(vars(3:7), funs(percent)) %>% bind_cols(epso_cov, act_dist) %>% mutate(`District Name`=districtname) %>%
  select(`District Name`,`Graduation Rate`, `% CTE`, `% BHN`,`% ED`, `% SWD`, `% EL`, `Avg. ACT (Highest)`, `% EPSO`) %>%  knitr::kable(col.names = c("District Name","Graduation Rate", "% CTE", "% BHN","% ED", "% SWD", "% EL", "Avg. ACT (Highest)", "% EPSO"),align="lccccccc", caption="2015 Graduates")
```
...
```{r subgroup_enrollment, fig.height = 9, fig.align = "center"}
subgroup_order <- dist %>%
...
```{r ACT_distribution, fig.height = 6, fig.width = 8}
dist %>%
    filter(page == "7a") %>%
...
```{r Figure9d, fig.height = 4.25, fig.width = 6}
dist %>%
    filter(page == "9d") %>%
...
```{r Figure9e, fig.height = 4.25, fig.width = 6}
dist %>%
    filter(page == "9e") %>%
    group_by(schoolno, epso_frpl_cat) %>%
    mutate(`Number Grads` = sum(hsgrad)) %>%
    filter(`Number Grads` >= 10 & institution_type!= "Did not enroll") %>%
    ungroup() %>%
