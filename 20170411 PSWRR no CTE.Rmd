...
dist %>%
  ...
  group_by(schoolname, subgroup) %>% 
    mutate(`Percent Enrolled` = sum(pct_enroll),`N Subgroup`=sum(hsgrad), `Percent Subgroup` = sum(new_perc)) %>% 
    ggplot(aes(x = subgroup, y = new_perc, fill = institution_type)) +
        geom_bar(stat = "identity", color = "black") +
        geom_text(aes(label = labPerc), vjust = 1.5, position = "stack") +
        geom_text(aes(x=subgroup , y= `Percent Subgroup`, label= paste(`Percent Enrolled`,"% enrolled \n Total N=",`N Subgroup`, sep = "")), vjust= -.75) + 
