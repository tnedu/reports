...
dist$pctElab = as.character(ifelse(dist$pct_enroll>= 5, dist$pct_enroll, ""))
dist$Elab = as.factor(ifelse(dist$hsgrad<10, "< 10", dist$enroll_bin))
...
dist %>%
     ...
     ggplot(aes(x = as.factor(GradYear ), y = pct_enroll, fill = as.factor(GradYear))) +
         ...
         geom_text(aes(label = paste(pctElab,"%\n N =",Elab, sep = ""), vjust = -.5)) +
         ...
         annotate("text", x="2014", y=95, 
                   label =paste("2015 TN 12 mo. Enrollment Rate: ", StateRate$pct_enroll_state, "% (Dashed line)", sep = ""),fontface="italic") +
...
all_districts %>%
    ...
    annotate("text",x= all_districts_lab$`Similar District`, y=95, label =paste("N=",all_districts_lab$`NumEnroll`, sep="")) +
  theme_hc() +
...
dist %>%
   ...
   mutate(pctElab = as.factor(ifelse(ps_pct_enroll>= 5 | hsgrad_denom<10, ps_pct_enroll, "*")), 
...
```{r subgroup Enrollment, fig.height = 3, fig.width = 4, fig.align = "center"}
...
dist %>%
    ...
    theme(axis.text.y = element_text(family = "serif"),
          ...
          plot.title = element_text(family = "serif")) +
    facet_grid(. ~ schoolname)  +
    scale_fill_manual(name = "Institution Type", values = c("#e02f11", "#24336b", "#959595","black"), limits=c("Four-year University", "Community College","Technical College", "Did not enroll")) +
